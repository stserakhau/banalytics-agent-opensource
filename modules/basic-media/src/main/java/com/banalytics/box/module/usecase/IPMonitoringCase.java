...
public class IPMonitoringCase extends AbstractUseCase<IPMonitoringCaseConfiguration> {
    Instance instance;

    public IPMonitoringCase(BoxEngine engine) {
        super(engine);
    }
    ...
    public void create() throws Exception {
        ...
        String streamUri = substitute(configuration.streamUri, configuration);
        URI uri = new URI(streamUri);
        String host = uri.getHost();
        double grabberMaxFps = 0;
        UrlMediaStreamThing camera = findUrlMediaStreamThing(host);
        if (camera == null) {
            camera = new UrlMediaStreamThing(engine);
            camera.getConfiguration().title = "Camera " + host;
            camera.getConfiguration().schema = MediaUrlSchema.valueOf(uri.getScheme());
            camera.getConfiguration().host = host;
            camera.getConfiguration().port = uri.getPort();
            camera.getConfiguration().path = uri.getPath();
            camera.getConfiguration().username = uri.getUserInfo();
            camera.getConfiguration().password = uri.getAuthority();
        ...
        {
            List<SimpleRTSPGrabberTask> grabberTasks = camera.findSubscriber(SimpleRTSPGrabberTask.class);
            SimpleRTSPGrabberTask grabberTask = grabberTasks.isEmpty() ? null : grabberTasks.get(0);
            if (grabberTask == null) {
                grabberTask = new SimpleRTSPGrabberTask(engine, instance);
                grabberTask.configuration.urlUuid = camera.getUuid();
    ...
    private UrlMediaStreamThing findUrlMediaStreamThing(String host) throws Exception {
        List<Thing<?>> fsList = engine.findThings(UrlMediaStreamThing.class);
        for (Thing<?> fs : fsList) {
            if (fs instanceof UrlMediaStreamThing lmt) {
    ...
    public static String substitute(String template, Object obj) {
        ...
        for (Field field : fields) {
            try {
                ...
                String fieldName = "{" + field.getName() + "}";  // Формируем строку для подстановки

                if (template.contains(fieldName)) {
                    // Выполняем подстановку
                    template = template.replace(fieldName, field.get(obj).toString());
                }
