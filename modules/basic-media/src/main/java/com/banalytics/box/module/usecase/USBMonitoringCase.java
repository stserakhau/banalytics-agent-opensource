...
public class USBMonitoringCase extends AbstractUseCase<USBMonitoringCaseConfiguration> {
    Instance instance;

    public USBMonitoringCase(BoxEngine engine) {
        super(engine);
    }
    ...
    public void create() throws Exception {
        ...
        LocalMediaThing camera = findLocalMediaThing(configuration.camera);
        if (camera == null) {
            camera = new LocalMediaThing(engine);
            camera.getConfiguration().title = "USB Camera " + camera.getUuid();
            camera.getConfiguration().camera = configuration.camera;
            {//          432x240/5.0-30.0
                String[] parts = configuration.resolution.split("/");
                String[] fpsParts = parts[1].split("-");
                Double minFps = Double.parseDouble(fpsParts[0]);
                Double maxFps = Double.parseDouble(fpsParts[1]);

                double targetFps = configuration.fps;
                if (targetFps < minFps) {
                    grabberMaxFps = targetFps;
                    targetFps = minFps;
                } else if (targetFps > maxFps) {
                    grabberMaxFps = targetFps;
                    targetFps = maxFps;
                }

                camera.getConfiguration().resolution = configuration.resolution;
                camera.getConfiguration().fps = targetFps;
            }
        ...
        {
            List<LocalMediaGrabberTask> grabberTasks = camera.findSubscriber(LocalMediaGrabberTask.class);
            LocalMediaGrabberTask grabberTask = grabberTasks.isEmpty() ? null : grabberTasks.get(0);
            if (grabberTask == null) {
                grabberTask = new LocalMediaGrabberTask(engine, instance);
                grabberTask.configuration.localMediaUuid = camera.getUuid();
    ...
    private LocalMediaThing findLocalMediaThing(String camera) throws Exception {
        List<Thing<?>> fsList = engine.findThings(LocalMediaThing.class);
        for (Thing<?> fs : fsList) {
            if (fs instanceof LocalMediaThing lmt) {
                if (lmt.getConfiguration().camera.equals(camera)) {
                    return lmt;
                }
            }
        }
