...
public final class ContinuousImageShotTask extends AbstractTask<ContinuousImageShotTaskConfig> implements FileStorageSupport {
    public ContinuousImageShotTask(BoxEngine metricDeliveryService, AbstractListOfTask<?> parent) {
    ...
    protected synchronized boolean doProcess(ExecutionContext executionContext) throws Exception {
        ...
        LocalDateTime nowTs = currentTimeInServerTz();

        boolean createImageShot = now >= nextPhotoTimeout;
            ...
            SystemThreadsService.execute(this, () -> {
                try (imageShot) {
                    ...
                    this.fileStorage.commitOutputTransaction(fileName, (dataPair) -> {
                        ...
                        evt.option("tsStart", nowTs);
                        evt.option("tsEnd", nowTs.plus(10, ChronoUnit.MILLIS));
